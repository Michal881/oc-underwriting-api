...
def enrich_oc_budowlane(record: PolicyRecord, text: str) -> None:
    ...
    record.covers_professional_liability = "no"
    record.covers_pure_financial_loss = yesno("czyste szkody majątkowe" in tl)
    record.has_construction_clause = yesno("przedsiębiorstw budowlanych" in tl)
    record.has_perimeter_clause = yesno("klauzula promienia" in tl)
    record.has_vibration_clause = yesno("wibracji" in tl or "osłabienia elementów nośnych" in tl)
    record.has_construction_machinery_clause = yesno("samobieżne maszyny budowlane" in tl or "dźwigi" in tl)
    record.has_property_under_control = yesno("rzeczy pod kontrolą" in tl)
    record.has_environment_damage = yesno("szkody w środowisku" in tl or "szkody ekologiczne" in tl)
    record.has_employer_liability = yesno("odpowiedzialność pracodawcy" in tl)
    record.has_travel_clause = yesno("podróże służbowe" in tl)
    record.has_vehicles_clause = yesno("pojazdy nie podlegające obowiązkowi ubezpieczenia" in tl)
    record.has_leased_property = yesno("wziętych w najem" in tl)


def enrich_oc_architekci(record: PolicyRecord, text: str) -> None:
    tl = lower_pl(text)
    record.covers_professional_liability = "yes"
    record.has_design_prof_liability = "yes"
    record.covers_pure_financial_loss = yesno("czystych szkód majątkowych" in tl or "czyste szkody majątkowe" in tl)
    record.has_building_damage_cover = yesno("szkoda w budynku lub budowli" in tl)
    record.has_gradual_damage_cover = yesno("szkody powstałe stopniowo" in tl)
    record.has_environment_damage = yesno("szkody w środowisku" in tl)
    record.has_cost_overrun_extension = yesno("przekroczeniem ustalonych kosztów" in tl)
    record.has_deadline_overrun_extension = yesno("przekroczeniem planowanego terminu budowy" in tl or "innych terminów" in tl)
    record.has_contractual_penalties_regress = yesno("kar umownych" in tl)


def determine_profession_subtype(text: str) -> str:
    tl = lower_pl(text)
    mapping = [
        ("broker", ["broker ubezpieczeniowy", "czynności zawodowe brokera"]),
        ("legal", ["świadczenie pomocy prawnej", "adwokatów i radców prawnych", "doradców podatkowych"]),
        ("accounting_consulting", ["prowadzenie ksiąg", "rachunkowych", "konsulting", "kadrowe"]),
        ("media", ["materiału prasowego", "media"]),
        ("advertising", ["agencji reklamowych", "reklam produktów i usług"]),
        ("property_management", ["zarządzaniu nieruchomościami", "zarządców nieruchomości"]),
    ]
    for label, keys in mapping:
        if find_all_keywords(tl, keys):
            return label
    return "other_professional"


def enrich_oc_zawodowe(record: PolicyRecord, text: str) -> None:
    tl = lower_pl(text)
    record.profession_subtype = determine_profession_subtype(text)
    record.covers_professional_liability = "yes"
    record.covers_pure_financial_loss = yesno("czyste szkody majątkowe" in tl)
    record.has_excess_layer = yesno(
        "nadwyżka ponad" in tl or "punkt zaczepienia" in tl or "klauzulą nadwyżkową" in tl
    )
    record.attachment_point = find_first([
        r"ponad\s+([0-9\s\.,]+(?:PLN|EUR|EURO|USD))",
        r"punkt zaczepienia\s*:?\s*(.*?)\s+(?:na jeden|na wszystkie|w okresie)",
    ], normalize(text))
    if "jednej czwartej sumy gwarancyjnej" in tl:
        record.documents_limit_rule = "quarter_of_sum_guaranteed"
    elif "do wysokości sumy gwarancyjnej" in tl and ("akt" in tl or "dokument" in tl):
        record.documents_limit_rule = "full_sum_guaranteed"


def validate_record(record: PolicyRecord) -> None:
    missing = []
    if record.product_type == "oc_ogolne":
        if not record.sum_guaranteed:
            missing.append("sum_guaranteed")
        if not record.insured_activity:
            missing.append("insured_activity")
    elif record.product_type == "oc_architekci":
        if not record.sum_guaranteed:
            missing.append("sum_guaranteed")
    elif record.product_type == "oc_zawodowe":
        if not record.sum_guaranteed:
            missing.append("sum_guaranteed")
        if not record.profession_subtype:
            missing.append("profession_subtype")
    elif record.product_type == "oc_budowlane":
        if not record.sum_guaranteed:
            missing.append("sum_guaranteed")

    record.data_quality_flag = "ok" if not missing else "missing:" + ",".join(missing)


def process_policy(path: str) -> Optional[PolicyRecord]:
    ...
    product_type, reasons = classify_policy(raw_text, filename)
    record = extract_common_fields(raw_text, filename, product_type, reasons)

    if product_type == "oc_ogolne":
        enrich_oc_ogolne(record, raw_text)
    elif product_type == "oc_budowlane":
        enrich_oc_budowlane(record, raw_text)
    elif product_type == "oc_architekci":
        enrich_oc_architekci(record, raw_text)
    elif product_type == "oc_zawodowe":
        enrich_oc_zawodowe(record, raw_text)

    validate_record(record)
    return record


def iter_policy_files(folder: str) -> List[str]:
    out: List[str] = []
...
def write_csv(records: List[PolicyRecord], output_path: str) -> None:
    if not records:
        print("Brak rekordów do zapisu.")
        return
...
def main() -> None:
    files = iter_policy_files(SOURCE_FOLDER)
    if MAX_FILES is not None:
        files = files[:MAX_FILES]

    print(f"Znaleziono plików do analizy: {len(files)}")
    ...
    for idx, path in enumerate(files, start=1):
        rec = process_policy(path)
        if rec is None:
            continue
        records.append(rec)
        counts[rec.product_type] = counts.get(rec.product_type, 0) + 1
        print(f"[{idx}/{len(files)}] {rec.source_file} -> {rec.product_type}")
